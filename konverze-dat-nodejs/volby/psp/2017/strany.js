...
function writeResults (json) {
  fs.writeFile("../data/volby/psp/2017/strany.json", JSON.stringify(json), function(err) {
...
var resultsFile = new Promise (function (resolve, reject) {
  fs.readFile('../zdroje/volby/psp/2017/psrkl-utf8.xml', function(err, data) {
...
Promise.all([allPartiesFile, resultsFile]).then(function (values) {
  ...
  allParties.forEach(party => {
    ...
    if (id.length > 0) {
      ...
      o.name = id[0].NAZEVPLNY[0];
      o.short = id[0].ZKRATKAK8[0];
      o.id = [];
