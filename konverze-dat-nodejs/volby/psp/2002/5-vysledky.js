...
var electedFiles = ['poslanci.html'];
var electedByParty = {};
  ...
  Object.keys(rows).forEach(row => {
    if (rows[row].name) {
      if (rows[row].children) {
        if (rows[row].children.filter(c => c.name).length === 11) {

          var id = rows[row].children.filter(c => c.name)[1].children[0].data;
...
Object.keys(resultCells).forEach(cellID => {
  ...
  var party = patriesInElection.list.find(p => (p.id ? p.id[0] : [9870]) === Number(cell.children[0].data));
