...
request.get('https://volby.cz/opendata/kv2014/KV2014reg20141014.zip')
        .pipe(unzipper.Parse())
        ...
        entry.pipe(iconv.decodeStream('windows-1250'))
          ...
          .pipe(fs.createWriteStream('../zdroje/volby/kom/2014/' + entry.path));
        })

request.get('https://volby.cz/opendata/kv2014/KV2014ciselniky20141008.zip')
        .pipe(unzipper.Parse())
        .on('entry', function (entry) {
          const fileName = entry.path;
...
function readXML (nuts) {
  request.get('https://volby.cz/pls/kv2014/vysledky_obce_okres?datumvoleb=20141010&nuts=' + nuts)
         .pipe(iconv.decodeStream('iso-8859-2'))
         .pipe(iconv.encodeStream('utf8'))
         .pipe(fs.createWriteStream('../zdroje/volby/kom/2014/data/' + nuts + ".xml"));
}
