...
request.get('https://volby.cz/opendata/ep2009/EP2009reg.zip')
        .pipe(unzipper.Parse())
...
request.get('https://volby.cz/opendata/ep2009/EP2009ciselniky.zip')
        .pipe(unzipper.Parse())
        .on('entry', function (entry) {
          const fileName = entry.path;
...
function readXML (nuts) {
  request.get('https://volby.cz/pls/ep2009/vysledky_okres?nuts=' + nuts)
         .pipe(iconv.decodeStream('iso-8859-2'))
         .pipe(iconv.encodeStream('utf8'))
         .pipe(fs.createWriteStream('../zdroje/volby/ep/2009/data/' + nuts + ".xml"));
}
