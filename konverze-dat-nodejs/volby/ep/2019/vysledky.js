...
function writeResults (json) {
  fs.writeFile("../data/volby/ep/2019/vysledky.json", JSON.stringify(json), function(err) {
...
var partiesFile = new Promise (function (resolve, reject) {
  fs.readFile('../data/volby/ep/2019/strany.json', function(err, data) {
    resolve(JSON.parse(data));
  });
});

var resultsFile = new Promise (function (resolve, reject) {
  fs.readFile('../zdroje/volby/ep/2019/vysledky-utf8.xml', function(err, data) {
    ...
    parser.parseString(bufReg, function (err, resultReg) {
        ...
        fs.readFile('../zdroje/volby/ep/2019/vysledky-utf8.xml', function(err, data) {
            ...
            parser.parseString(buf, function (err, result) {
              ...
              var json = {
                votes: Number(result.VYSLEDKY.CR.UCAST[0].$.PLATNE_HLASY),
                voters: Number(result.VYSLEDKY.CR.UCAST[0].$.ZAPSANI_VOLICI),
                attended: Number(result.VYSLEDKY.CR.UCAST[0].$.VYDANE_OBALKY),
                parties: []
              };

              result.VYSLEDKY.CR.STRANA.forEach(function (strana) {
