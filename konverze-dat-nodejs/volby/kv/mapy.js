...
var electionList = [2008, 2012, 2016];
  ...
  if (regions) {
    regions.list.forEach(region => {
      ...
      if (region.results) {
        ...
        region.results.parties.forEach(result => {
          var party = parties.find(p => p.reg === result.reg);
        ...
        region.results.parties.sort((a, b) => b.pct - a.pct);
        ...
        for (var i = 0; i < 5; i++) {
          if (region.results.parties[i]) {
            var p = {
              reg: region.results.parties[i].reg,
              pct: region.results.parties[i].pct
            }
